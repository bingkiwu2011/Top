/** 
* @Title: MyTopCometMessageListener.java 
* @Package com.top.info 
* @Description: TODO(用一句话描述该文件做什么) 
* Copyright: Copyright (c)2009 
* Company:上海建周 
* @author bingki 
* @date 2012-8-28 下午3:02:53 
*@version 1.0 
*/
...
/** 
 * @ClassName: MyTopCometMessageListener 
 * @Description: TODO(这里用一句话描述这个类的作用) 
 * @author bingki 
 * @date 2012-8-28 下午3:02:53  
 */
public class MyTopCometMessageListener implements TopCometMessageListener {
	private static Log log = LogFactory.getLog(MyTopCometMessageListener.class);
	/**
	...
	public void onConnectMsg(String message){
		if(log.isDebugEnabled())
			log.debug("onConnectMsg "+message);
	}
	/**
	...
	public void onReceiveMsg(String message){
		log.info("onReceiveMsg:"+message);
		JSONObject jsonObject;
		try {
			jsonObject = JSON.parseObject(message);
			if(jsonObject.getString("notify_topats").length()>0){
				String msgString=jsonObject.getString("notify_topats");
				TaskNotify th=new TaskNotify(msgString);
				Thread thread=new Thread(th);
				thread.start();
			}else{
				ProductNotify th=new ProductNotify(message);
				Thread thread=new Thread(th);
				thread.start();
			}
		} catch (JSONException e) {
			log.error("error onReceiveMsg:"+message,e);
		}
	}
	/**
	...
	public void onServerUpgrade(String message){
		if(log.isWarnEnabled())
			log.warn("onServerUpgrade:"+message);
	}
	/**
