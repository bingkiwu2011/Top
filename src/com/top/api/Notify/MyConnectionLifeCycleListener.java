/** 
* @Title: MyConnectionLifeCycleListener.java 
* @Package com.top.info 
* @Description: TODO(用一句话描述该文件做什么) 
* Copyright: Copyright (c)2009 
* Company:上海建周 
* @author bingki 
* @date 2012-8-28 下午2:59:18 
*@version 1.0 
*/
package com.top.api.Notify;

import org.apache.commons.logging.Log;
import org.apache.commons.logging.LogFactory;

import com.taobao.api.internal.stream.connect.ConnectionLifeCycleListener;

/** 
 * @ClassName: MyConnectionLifeCycleListener 
 * @Description: TODO(这里用一句话描述这个类的作用) 
 * @author bingki 
 * @date 2012-8-28 下午2:59:18  
 */
public class MyConnectionLifeCycleListener implements ConnectionLifeCycleListener {
	private static Log log = LogFactory.getLog(MyConnectionLifeCycleListener.class);
	/**
	 * 在每次发起连接请求之前，允许应用程序做一些事情。
	 */
	public void onBeforeConnect() {

	}
	/**
	 * 系统中出现的任何与连接相关的异常时调用此方法，这里包括连接过程中发生的异常，和系统处理过程中发生的异常</br>
	 * 1，连接异常,签名错误，丢失参数等异常时，抛出这个异常后，表名没有正确连接到服务端</br>
	 * 2，系统处理过程中可能会抛出一些异常</br>
	 * isv可以关注此异常信息，用于诊断系统运行状况
	 * 
	 * @param e 包含了系统运行过程中错误的信息。
	 */
	public void onException(Throwable e) {		
		log.error(e);
	}

	/**
	 * 当系统在30分钟内超过10次timeout了,则调用这个方法，sdk会继续重练
	 * 但是强烈建议isv监控此方法，当频繁出现readtimeout的时候，说明
	 * 网络环境可能不是很稳定，需要人工介入检查一下是不是网络有问题。
	 * 
	 */
	public void onMaxReadTimeoutException() {
		// TODO Auto-generated method stub
		log.warn("在30分钟内超过10次timeout!");
	}

}
